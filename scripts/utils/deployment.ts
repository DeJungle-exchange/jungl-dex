...
async function deployOrUpgrade(
  ...
  if (existing && addresses[name]) {
    const implementationAddress =
      await upgrades.erc1967.getImplementationAddress(addresses[name]);
    ...
    if (
      ...
      deployedBytecode.length /*|| contractName === 'Jungle'*/
    ) {
...
async function deployOrRedeploy(
  ...
  if (contract.newlyDeployed) {
    writeVerifyToFile(
      verifyFile,
      `\n npx hardhat verify ${[addresses[name], ...input]
        .map((x) => x.toString())
