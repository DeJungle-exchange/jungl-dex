...
contract Voter is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setEpochController(address _epochControllerAddress) external {
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) {
            // gov can create for any pool, even non-Jungl pairs
            ...
            require(
                zusd == address(0) || tokenA == zusd || tokenB == zusd,
                "!zusd"
            );
        }
        ...
        string memory _type = string.concat(
            "Jungl LP Fees: ",
            IERC20MetadataUpgradeable(_pool).symbol()
        );
