...
contract Minter is IMinter, OwnableUpgradeable {
    ...
    function initialize(
        ...
        _jungl = IJungl(IVotingEscrow(__ve).token());
        _voter = IVoter(__voter);
        _ve = IVotingEscrow(__ve);
        _rewards_distributor = IRewardsDistributor(__rewards_distributor);

        active_period = (block.timestamp / EPOCH_DURATION) * EPOCH_DURATION;

        weekly = 25_000_000 * 1e18; // represents a starting weekly emission of 25M JUNGL
        isFirstMint = true;
    }
    ...
    function calculate_rebase(
        ...
        uint256 _veTotal = _jungl.balanceOf(address(_ve));

        uint256 _junglTotal = _jungl.totalSupply();

        uint256 lockedShare = (_veTotal * REBASESLOPE) / _junglTotal;
    ...
    function update_period() external returns (uint256) {
        ...
        if (
            ...
            uint256 _balanceOf = _jungl.balanceOf(address(this));
            if (_balanceOf < _required) {
                _jungl.mint(address(this), _required - _balanceOf);
            }

            require(_jungl.transfer(team, _teamEmissions));

            require(_jungl.transfer(address(_rewards_distributor), _rebase));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _jungl.approve(address(_voter), _gauge);
