...
contract Pair is IPair, Initializable {
    ...
    function swap(
        ...
        require(amount0In != 0 || amount1In != 0, "IIA"); // Pair: INSUFFICIENT_INPUT_AMOUNT
        {
            ...
            // do not deduct fee for whitelisted AMO
            if (!IPairFactory(factory).isAMO(msg.sender)) {
                _updateFees(
                    IPairFactory(factory).getFeeAmount(
                        stable,
                        amount0In,
                        msg.sender
                    ),
    ...
    function getAmountOut(
        ...
        (uint256 _reserve0, uint256 _reserve1) = (reserve0, reserve1);
        amountIn -= IPairFactory(factory).getFeeAmount(
            stable,
            amountIn,
            msg.sender
